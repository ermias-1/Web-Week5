...
// so can draw it to the canvas and enable the input 
 let image = new Image(); 
 image.src = "King Lion.jpg"; 
 
 
 
 image.addEventListener("load", function () { 
    context.drawImage(image, 0, 0);      input.disabled = false; 
 }) 

 
// Add event listener to input - will fire if the text is changed 
 input.addEventListener("input", function() {      // Read the data typed in the input      // 'this' is the HTML element this function is called for  
    let text = this.value; 
